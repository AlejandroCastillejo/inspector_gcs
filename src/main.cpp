...
// std::list<std::string> uav_list = {"uav_1", "uav_2", "uav_3", "uav_n"};
...
/* 
  // TEST
  qDebug() << endl;
  qDebug() << "n Drones:" << h_d.size() << endl;
    for (int i=0; i<n; i++) {
        qDebug() << " drone" << i << endl<< "positioning height" << h_d[i] << endl << "way points NED: " << droneWayPointsNED[i];
        qDebug() << endl << "way points Geo: " << droneWayPointsGeo[i] << endl;
        // qDebug() << h_d[i];
    } 
*/ 
  // visualization();
...
bool create_mission_cb(inspector_gcs::gcsCreateMission::Request &req, inspector_gcs::gcsCreateMission::Response &res) {
  ...
  // Test
    std::cout << "\n PolygonCoordinates \n" << std::endl;
    std::cout << "altura de vuelo: " << h_c << std::endl;
  ...
  BaseCoordinate = QGeoCoordinate{4.520000, -5.600000};
